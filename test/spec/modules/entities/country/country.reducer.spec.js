...
test('attempt searching a country', () => {
  const state = reducer(INITIAL_STATE, Actions.countrySearchRequest(1))
...
test('success searching a country', () => {
  const state = reducer(INITIAL_STATE, Actions.countrySearchSuccess({ id: 1 }))

  expect(state.searching).toBe(false)
  expect(state.errorSearching).toBe(null)
  expect(state.countries).toEqual({ id: 1 })
})
...
test('failure searching a country', () => {
  const state = reducer(INITIAL_STATE, Actions.countrySearchFailure({ error: 'Not found' }))
