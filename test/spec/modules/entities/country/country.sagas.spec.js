...
import {
  ...
  deleteCountry,
  searchCountries,
} from '../../../../../app/modules/entities/country/country.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchCountries()
  const step = stepper(searchCountries(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(CountryActions.countrySearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(CountryActions.countrySearchFailure()))
})
