...
import {
  ...
  deleteUserDetail,
  searchUserDetails,
} from '../../../../../app/modules/entities/user-details/user-details.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchUserDetails()
  const step = stepper(searchUserDetails(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(UserDetailActions.userDetailSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(UserDetailActions.userDetailSearchFailure()))
})
