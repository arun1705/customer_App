...
test('attempt searching a userDetail', () => {
  const state = reducer(INITIAL_STATE, Actions.userDetailSearchRequest(1))
...
test('success searching a userDetail', () => {
  const state = reducer(INITIAL_STATE, Actions.userDetailSearchSuccess({ id: 1 }))

  expect(state.searching).toBe(false)
  expect(state.errorSearching).toBe(null)
  expect(state.userDetails).toEqual({ id: 1 })
})
...
test('failure searching a userDetail', () => {
  const state = reducer(INITIAL_STATE, Actions.userDetailSearchFailure({ error: 'Not found' }))
